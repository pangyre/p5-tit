...
package Tit v0.0.1 {
    ...
    use Carp;
    #require Tit::Request;
    #require Tit::Response;
    use feature "state";
    our $AUTHORITY  = "cpan:ASHLEY";
    our $REPOSITORY = "https://bitbucket.org/pangyre/wren/";
    our @ISA;
    push @ISA, "Exporter";
    our @EXPORT = qw( get put post del add_view add_model );
    sub tit; my %tits; no warnings "redefine";

    my $imported;
    sub tit { state $tit ||= shift };
    sub import {
        croak "You cannot use Tit twice" if tit;
        __PACKAGE__->export_to_level(1, @_);
        tit( initialize() );
        # warn "blessed -> ", blessed(tit);
    }

    has router =>
        is => "lazy",
        isa => sub { blessed $_[0] eq "Tit::Router"
                         or croak "Only router allowed is Tit::Router" },
        default => sub { "Tit::Router"->new }
        ;

    sub add_route {
        my $self = +shift->router->add_route(@_);
    }
    ...
    sub initialize {
        ...
        {
            $ns = ( caller($_) )[0];
            last unless ( caller( $_ + 1 ) )[0];
            last if "main" eq ( caller( $_ + 1 ) )[0];
        }
        $ns->new;
    }
    ...
    sub get($&;$) {
        ...
        my $code = shift; # Should be able to omit!?! Only for HTML view, right? So, no?
        tit->add_route({ method => "GET",
                         definition => $uri,
                         code => $code });
    }
    ...
    sub to_app {
        ...
        sub {
            my $env = shift;
            $tit->set_request( my $req = Tit::Request->new($env) );
            $tit->set_response( my $res = Tit::Response->new );
            # use YAML; warn YAML::Dump($req);
            # warn "HERE -> ", $res || $@;
            # use YAML; warn YAML::Dump($res);
            my $asdf = join "", $req->method, $req->path;
            warn $asdf;
    ...
    #my $CALLER;
    # $CALLER ||= blessed $class || $class;
    #sub package { $CALLER || croak "Initiating package was not set! use Tit, not require Tit!" }
    #sub tit {
    #    for ( 0 .. 12 ) # Padding in case.
    #    {
    #        warn "Caller $_ 0 -> ", ( caller($_) )[0];
    #        last unless ( caller( $_ ) )[0];
    #    }
    #}
...
package Tit::Route {
    ...
    has definition =>
        is => "ro",
        isa => sub {}, # Static or regex…
        required => 1,
        ;

    # my $dispatch = join "/", GET => $uri;
    1;
};
...
package Tit::Router {
    ...
    use MooX::HandlesVia;
    use strictures;
    use Types::Standard qw( ArrayRef HashRef InstanceOf Object );
    use Scalar::Util "blessed";
    use Carp;

    #my $type = ArrayRef[ # Also doesn't work with handles…
    #    Object->plus_coercions( HashRef, q{Tit::Route->new($_)} )
    #    ];

    has _routes =>
        is => "rw",
        isa => ArrayRef[ InstanceOf['Tit::Route'] ], # Doesn't help at handles level…
        default => sub { [] },
        handles_via => "Array", # https://metacpan.org/pod/Data::Perl::Role::Collection::Array
        handles => {
            route => "push",
            routes => "elements",
    };
    ...
    around route => sub {
        my $orig = shift;
        my $self = shift;
        my $route = shift || croak "Route required";
        # Doesn't check type…
        $self->$orig( blessed $route ? $route : Tit::Route->new($route) );
    };
...
=cut
    ...
    sub _build_default_view {
        ...
        {
            $self->view("Text") || $self->view($views[0]);
        }
        else
        {
            require Tit::View::Text;
            Tit::View::Text->new;
        }
    }
        ...
        my $APP = sub {
            ...
            eval {
                ...
                if ( $route )
                {
                    ...
                    $res->_set_request($req); # More like LWP.
                }
                else
                {
            ...
            if ( $res->charset && $res->body && $req->accept ne "JSON" )
            {
                ...
                $_ = encode( $res->charset, $_, Encode::FB_CROAK ) for @{ $res->body };
            }
